...
template <typename T> LList<T>::Item::Item(const T& v, Item* p, Item* n)
: val(v), prev(p), next(n)
{
}
...
template <typename T> LList<T>::LList(const LList& other) {
  mHead = new Item(T(), nullptr, nullptr);
  mTail = mHead;
  mSize = 0;
...
template <typename T> LList<T>& LList<T>::operator=(const LList& other) {
  if(this == &other){
    return *this;
  }
  clear();
...
template <typename T> void LList<T>::push_back(const T& val) {
...
template <typename T> T& LList<T>::get(int loc) {
  Item *temp = getNodeAt(loc);
  return temp->val;
}

template <typename T> const T& LList<T>::get(int loc) const {
  Item *temp = getNodeAt(loc);
  return temp->val;
}
...
template <typename T> typename LList<T>::Item* LList<T>::getNodeAt(int loc) const {
  if(loc >= 0 && loc < mSize){
    Item *temp = mHead;
    while(loc > 0){
      temp = temp->next;
